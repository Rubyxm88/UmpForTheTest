...
import { buildWeeklyPlaylist, explainWeeklyPlaylist } from './weekly-playlist.js';
import { formatWeeklyAbOutcomeText } from './weekly-ab-utils.js';
...
import {
  ...
  getIsoWeekKey,
  formatWeekLabel,
  getWeeklyLeaderboardPeriodKey,
  isWeeklyProgressCompleteForLeaderboard,
...
import {
  pickNextStreakAb,
  streakCorrectCount as computeStreakCorrectCount,
  getStreakDateKey,
  streakRunSeed,
  getAbById,
  getStreakPool,
...
let streakSessionUsedIds = new Set();
let streakDaySeenIds = new Set();
let streakRunSeedValue = 0;
...
let streakSummaryFinalStreak, streakSummaryAccuracy, streakSummaryFraction, streakSummaryUmpAccuracy, streakSummaryUmpFraction;
...
let streakTransitionOverlay, streakTransitionOutcome, streakTransitionCurrent;
let streakTransitionTodayBest, streakTransitionAlltimeBest, streakTransitionRank;
let streakTransitionNextHint, btnStreakTransitionContinue;
...
function hideStreakSummaryXpPopover() {
  const pop = document.getElementById('streak-summary-xp-popover');
  const btn = document.getElementById('streak-summary-xp-earned');
  if (pop) pop.classList.add('hidden');
  if (btn) btn.setAttribute('aria-expanded', 'false');
}
...
function toggleStreakSummaryXpPopover() {
  const pop = document.getElementById('streak-summary-xp-popover');
  const btn = document.getElementById('streak-summary-xp-earned');
...
function renderStreakSummaryXpPopover(correctCount, totalXp) {
  const list = document.getElementById('streak-summary-xp-popover-list');
  const totalEl = document.getElementById('streak-summary-xp-popover-total');
  if (!list) return;

  const pitchXp = correctCount * 10;
  const runBonus = Math.max(0, totalXp - pitchXp);

  list.innerHTML = '';
  const rows = [
    { label: `Correct calls (${correctCount})`, pts: `+${pitchXp}`, sub: '10 XP each' },
  ];
  if (runBonus > 0) {
    rows.push({ label: 'Streak run bonus', pts: `+${runBonus}`, sub: 'End of run' });
  }
...
function handleSummaryXpPopoverOutsideClick(e) {
  const abWrap = document.getElementById('ab-summary-xp-earned')?.closest('.ab-summary-xp-earned-wrap');
  const streakWrap = document.getElementById('streak-summary-xp-earned')?.closest('.ab-summary-xp-earned-wrap');
  if (abWrap && !abWrap.contains(e.target)) hideAbSummaryXpPopover();
  if (streakWrap && !streakWrap.contains(e.target)) hideStreakSummaryXpPopover();
}
...
function bindAbSummaryXpPopoverOnce() {
  ...
  document.addEventListener('click', handleSummaryXpPopoverOutsideClick);
}

let streakSummaryXpPopoverBound = false;
function bindStreakSummaryXpPopoverOnce() {
  if (streakSummaryXpPopoverBound) return;
  streakSummaryXpPopoverBound = true;
  const btn = document.getElementById('streak-summary-xp-earned');
  if (btn) {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      toggleStreakSummaryXpPopover();
    });
  }
}
...
function mergeUserStatsForUpload(local = {}, cloud = {}) {
  ...
  const merged = {
    ...
    challengeProgress: pickNewerChallengeProgress(local.challengeProgress, cloud.challengeProgress),
  };
...
function cacheDOM() {
  ...
  streakSummaryUmpAccuracy = document.getElementById('streak-summary-ump-accuracy');
  streakSummaryUmpFraction = document.getElementById('streak-summary-ump-fraction');
  ...
  bindStreakSummaryXpPopoverOnce();

  streakTransitionOverlay = document.getElementById('streak-transition-overlay');
  streakTransitionOutcome = document.getElementById('streak-transition-outcome');
  streakTransitionCurrent = document.getElementById('streak-transition-current');
  streakTransitionTodayBest = document.getElementById('streak-transition-today-best');
  streakTransitionAlltimeBest = document.getElementById('streak-transition-alltime-best');
  streakTransitionRank = document.getElementById('streak-transition-rank');
  streakTransitionNextHint = document.getElementById('streak-transition-next-hint');
  btnStreakTransitionContinue = document.getElementById('btn-streak-transition-continue');
  ...
  if (btnStreakSummaryAdvance) {
    btnStreakSummaryAdvance.addEventListener('click', () => {
      ...
      startDailyStreakChallenge(false, { forceNewRun: true });
    });
  }
  ...
  if (btnStreakTransitionContinue) {
    btnStreakTransitionContinue.addEventListener('click', () => {
      initAudio();
      continueStreakFromTransition();
    });
  }
  ...
  if (streakSummaryOverlay) {
    streakSummaryOverlay.addEventListener('pointerdown', (e) => {
      const popout = document.querySelector('.player-card-popout');
      if (!popout) return;
      const trigger = popout.closest('.ab-summary-player');
      if (trigger?.contains(e.target) || popout.contains(e.target)) return;
      closeAllPlayerStatsPopouts();
    });
  }
...
function attachEvents() {
  ...
  if (btnMainMenu) {
    btnMainMenu.addEventListener('click', async () => {
      if (gameMode === 'daily_streak' && !isSessionOver) {
        const confirmed = await showCustomConfirm("Leaving forfeits this streak run. Exit anyway?");
        if (!confirmed) return;
        await forfeitStreakRun();
      }
      goToMainMenu();
    });
  }
  ...
  document.querySelectorAll('#welcome-login-fields > .flex.flex-col').forEach((fieldWrap) => {
    const fieldInput = fieldWrap.querySelector('input');
    if (!fieldInput) return;
    fieldWrap.addEventListener('pointerdown', (e) => {
      if (e.target === fieldInput) return;
      e.preventDefault();
      fieldInput.focus();
    });
  });
  ...
  if (leaderBtnWeekly) {
    leaderBtnWeekly.addEventListener('click', () => {
      ...
      activeWeeklyPeriodKey = getActiveWeeklyLeaderboardPeriodKey();
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (abSummaryOverlay && abSummaryOverlay.classList.contains('opacity-100')) {
      if (e.key === ' ') {
        e.preventDefault();
        void advanceNextAtBat();
      }
      return;
    }

    // Check if Streak transition is open
    if (streakTransitionOverlay && streakTransitionOverlay.classList.contains('opacity-100')) {
      if (e.key === ' ') {
        e.preventDefault();
        initAudio();
        continueStreakFromTransition();
    ...
    case STATES.IDLE:
      ...
      if (pitchesList.length === 0) {
        if (gameMode === 'standard') {
          ...
          if (inningCard) inningCard.classList.add('hidden');
          if (pitchesList.length === 0 && currentStreakAbMeta?.pitches?.length) {
            pitchesList = currentStreakAbMeta.pitches;
          }
          currentPitchIndex = Math.min(currentPitchIndex, Math.max(0, pitchesList.length - 1));
        }
      }
      ...
      if (gameMode === 'standard') {
        ...
        const streakLen = computeStreakCorrectCount(streakPitchHistory);
        pitchCounterText.textContent = `STREAK: ${streakLen} | PITCH ${currentPitchIndex + 1}`;
      ...
      if (userHistoryItem.isSwingPlay) {
        ...
        if (abEnded) {
          ...
          if (gameMode === 'daily_streak') {
            const revealMs = 1200;
            if (isSessionOver) {
              scheduleStreakPostPitchFlow(() => showStreakSummaryScreen(), revealMs);
            } else {
              scheduleStreakPostPitchFlow(() => finishAtBatAndShowSummary(), revealMs);
            }
          } else {
        ...
        } else if (gameMode === 'daily_streak' && isSessionOver) {
          hideQuickPreviewPanel();
          scheduleStreakPostPitchFlow(() => showStreakSummaryScreen(), minPreviewMs);
        } else {
        ...
        if (reviewStyle === 'quick' || !loggedIn) {
          ...
          if (!loggedIn) {
            ...
            if (gameMode === 'daily_streak') {
              if (isSessionOver) {
                scheduleStreakPostPitchFlow(() => showStreakSummaryScreen(), summaryRevealMs);
              } else {
                scheduleStreakPostPitchFlow(() => finishAtBatAndShowSummary(), summaryRevealMs);
              }
            } else {
              if (summaryTimeout) clearTimeout(summaryTimeout);
              isTransitioningToSummary = true;
            ...
            hideQuickPreviewPanel();
            scheduleStreakPostPitchFlow(() => showStreakSummaryScreen(), effectiveMinPreviewMs);
          } else {
            if (rateOfPlay !== 'updated' || isCorrect) {
              showQuickPreviewDock();
            }
...
function updateCountAndCheckABEnd(historyItem) {
  ...
  if (historyItem.isSwingPlay && historyItem.swingOutcome === 'HIT') {
    ...
    abEnded = true;
    if (gameMode === 'weekly_challenge') {
      abOutcomeText = formatWeeklyAbOutcomeText(currentPitch, {
        balls: abBalls,
        strikes: abStrikes,
      });
    } else {
...
function getStreakDaySeenStorageKey() {
  const username = localStorage.getItem('ump_username') || 'GUEST';
  return `streak_day_seen_${getStreakDateKey()}_${username.toUpperCase()}`;
}

function getStreakDayAttemptStorageKey() {
  const username = localStorage.getItem('ump_username') || 'GUEST';
  return `streak_day_attempt_${getStreakDateKey()}_${username.toUpperCase()}`;
}

function loadStreakDaySeenIdsFromStorage() {
  try {
    const raw = localStorage.getItem(getStreakDaySeenStorageKey());
...
function persistStreakDaySeenIds() {
  localStorage.setItem(getStreakDaySeenStorageKey(), JSON.stringify([...streakDaySeenIds]));
}

function recordStreakDaySeenIds(ids) {
  if (!ids?.size) return;
  ids.forEach((id) => streakDaySeenIds.add(id));
  persistStreakDaySeenIds();
}

async function clearStreakChallengeSession(username = localStorage.getItem('ump_username')) {
  if (username && username !== 'GUEST_UMPIRE') {
    try {
      await clearActiveSession(username);
    } catch (err) {
      console.warn('Failed to clear streak IndexedDB session:', err);
    }
  }
...
function initNewStreakRunSeed() {
  const username = localStorage.getItem('ump_username') || 'GUEST';
  const dateKey = getStreakDateKey();
  const attemptKey = getStreakDayAttemptStorageKey();
  const attemptNo = (parseInt(localStorage.getItem(attemptKey) || '0', 10) || 0) + 1;
  localStorage.setItem(attemptKey, String(attemptNo));
  streakRunSeedValue = streakRunSeed(dateKey, username, attemptNo);
  streakDaySeenIds = loadStreakDaySeenIdsFromStorage();
}

function cancelPendingStreakFlowTimers() {
  if (autoPlayTimeout) {
    clearTimeout(autoPlayTimeout);
    autoPlayTimeout = null;
  }
...
function syncStreakSessionOverFromHistory() {
  if (gameMode !== 'daily_streak') return false;
  const called = streakPitchHistory.filter((x) => !x.isSwingPlay);
  const last = called[called.length - 1];
  if (last && !last.userCorrect) {
    isSessionOver = true;
    activeAbEnded = true;
    return true;
  }
...
function isStreakBetweenAbMenuVisible() {
  return abStartOverlay && abStartOverlay.classList.contains('opacity-100');
}

function scheduleStreakPostPitchFlow(fn, delayMs) {
  const epoch = streakFlowEpoch;
  if (summaryTimeout) clearTimeout(summaryTimeout);
  isTransitioningToSummary = true;
  summaryTimeout = setTimeout(() => {
    summaryTimeout = null;
    isTransitioningToSummary = false;
    if (epoch !== streakFlowEpoch) return;
    if (gameMode !== 'daily_streak') return;
    if (isStreakBetweenAbMenuVisible() && !isSessionOver) return;
    fn();
  }, delayMs);
}
 ...
 */
function finishAtBatAndShowSummary(flowEpoch) {
  if (flowEpoch !== undefined && flowEpoch !== streakFlowEpoch) return;

  syncStreakSessionOverFromHistory();
...
function showAtBatStartScreen(onConfirmCallback, isResume = false, startOpts = {}) {
  ...
  // Cancel any running auto-play or stale post-pitch timers
  cancelPendingStreakFlowTimers();
  ...
  const startStreakDetails = document.getElementById('ab-start-streak-details');
  const streakCurrentEl = document.getElementById('ab-start-streak-current');
  const streakTodayEl = document.getElementById('ab-start-streak-today-best');
  const streakAlltimeEl = document.getElementById('ab-start-streak-alltime-best');
  const streakRankEl = document.getElementById('ab-start-streak-rank');
  const streakDifficultyWrap = document.getElementById('ab-start-streak-difficulty');
  const streakLastOutcomeEl = document.getElementById('ab-start-streak-last-outcome');

  // Always hard-hide the irrelevant dock (both exist in DOM).
  if (startWeeklyDetails) startWeeklyDetails.classList.toggle('hidden', gameMode !== 'weekly_challenge');
  if (startStreakDetails) startStreakDetails.classList.toggle('hidden', gameMode !== 'daily_streak');
  if (streakDifficultyWrap) streakDifficultyWrap.classList.toggle('hidden', gameMode !== 'daily_streak');
  if (streakLastOutcomeEl && gameMode !== 'daily_streak') streakLastOutcomeEl.classList.add('hidden');
  ...
  if (gameMode === 'weekly_challenge' && weeklyPlaylistABs && weeklyPlaylistABs.length > 0) {
    ...
    if (startStreakDetails && gameMode !== 'daily_streak') {
      startStreakDetails.classList.add('hidden');
    }
    if (startSubtitle) {
      ...
      if (gameMode === 'daily_streak') {
        ...
        if (startStreakDetails) startStreakDetails.classList.remove('hidden');
        if (streakCurrentEl) streakCurrentEl.textContent = String(correct);
        if (streakDifficultyWrap) {
          streakDifficultyWrap.classList.remove('hidden');
          updateAbStartStreakDifficultyUi(currentStreakAbMeta);
        }
        if (streakLastOutcomeEl) {
          if (lastAbOutcomeText && streakSessionUsedIds.size > 1) {
            streakLastOutcomeEl.textContent = lastAbOutcomeText;
            streakLastOutcomeEl.classList.remove('hidden');
          } else {
            streakLastOutcomeEl.textContent = '';
            streakLastOutcomeEl.classList.add('hidden');
          }
        }

        const username = localStorage.getItem('ump_username') || 'GUEST_UMPIRE';
        const statsKey = getStatsStorageKey(username);
        const stats = JSON.parse(localStorage.getItem(statsKey) || '{"maxStreak":0,"streakHistory":{}}');
        const todayStr = new Date().toISOString().slice(0, 10);
        const todayBest = (stats.streakHistory && stats.streakHistory[todayStr]) || 0;
        if (streakTodayEl) streakTodayEl.textContent = String(Math.max(todayBest, correct));
        if (streakAlltimeEl) streakAlltimeEl.textContent = String(Math.max(stats.maxStreak || 0, correct));
        if (streakRankEl) {
          streakRankEl.textContent = username !== 'GUEST_UMPIRE' ? '…' : '—';
          if (username !== 'GUEST_UMPIRE') {
            getLeaderboardRows('daily', username)
              .then(({ rows }) => {
                const me = rows.find((r) => r.isUser);
                streakRankEl.textContent = me ? `#${me.rank}` : '—';
              })
              .catch(() => {
                streakRankEl.textContent = '—';
              });
          }
        }
      } else {
  ...
  // Mode-specific body class for sizing/styling
  document.body.classList.toggle('streak-mode', gameMode === 'daily_streak');
...
function updateLiveScoreboard() {
  ...
  if (gameMode === 'standard') {
    ...
    pitchCounterText.textContent = `STREAK: ${computeStreakCorrectCount(streakPitchHistory)} | PITCH ${currentPitchIndex + 1}`;
  } else {
    pitchCounterText.textContent = `PITCH ${totalPitchesCount + 1} | OUTS ${inningOuts}`;
  }
...
function renderScoreboardDashboard() {
  ...
  if (username) {
    ...
    const weeklyRunComplete =
      gameMode === 'weekly_challenge' && isWeeklyChallengeRunComplete(weeklyPlaylistABs);
    if (weeklyRunComplete) {
    ...
    // Leaderboard submit — weekly only after all ABs are complete; streak submits per run
    if (weeklyRunComplete) {
 ...
 */
async function startDailyStreakChallenge(isResume = false, opts = {}) {
  if (!isResume && !requireLoggedInUser()) return;
  const forceNewRun = opts.forceNewRun === true;
  try {
  const username = localStorage.getItem('ump_username') || 'GUEST_UMPIRE';
  updateDailyStreakStatusUI();
  hideChallengeDetailModal();
  ...
  if (!STREAK_POOL_META?.totalAbs) {
    throw new Error('Streak pool not built — run npm run streak-refresh');
  }
  
  let restored = false;

  if (!isResume && !forceNewRun && username && username !== 'GUEST_UMPIRE') {
    try {
      const session = await getActiveSession(username);
      if (session?.gameMode === 'daily_streak' && (session.currentStreakAbMeta || session.streakPitchHistory?.length)) {
        streakPitchHistory = session.streakPitchHistory || [];
        streakSessionUsedIds = new Set(session.streakSessionUsedIds || []);
        currentStreakAbMeta = session.currentStreakAbMeta || null;
        activeStreakAbIndex = session.activeStreakAbIndex || 0;
        streakRunSeedValue = session.streakRunSeedValue || 0;
        streakDaySeenIds = new Set(session.streakDaySeenIds || loadStreakDaySeenIdsFromStorage());
    ...
    if (!restored) {
      ...
      if (active?.gameMode === 'daily_streak' && (active.streakPitchHistory?.length || savedData.streakSessionUsedIds?.length)) {
        streakPitchHistory = active.streakPitchHistory || [];
        streakSessionUsedIds = new Set(savedData.streakSessionUsedIds || active.streakSessionUsedIds || []);
        currentStreakAbMeta = savedData.currentStreakAbMeta || active.currentStreakAbMeta || null;
        streakRunSeedValue = savedData.streakRunSeedValue || active.streakRunSeedValue || 0;
        streakDaySeenIds = new Set(savedData.streakDaySeenIds || active.streakDaySeenIds || loadStreakDaySeenIdsFromStorage());
        // Normalize AB index from session-used set (prevents off-by-one on resume).
...
function loadNextStreakAtBat(isResume = false) {
  let abData = isResume && currentStreakAbMeta ? currentStreakAbMeta : null;

  if (!abData) {
    const streakLen = computeStreakCorrectCount(streakPitchHistory);
    abData = pickNextStreakAb(streakLen, streakSessionUsedIds, getStreakDateKey(), {
      runSeed: streakRunSeedValue || undefined,
      daySeenIds: streakDaySeenIds,
    });
    if (!abData?.pitches?.length) {
      throw new Error('No eligible streak at-bats available for today');
    }
  ...
  if (!isResume) {
    ...
    if (checkReconstructedAbCompleted()) {
      console.log('Resumed streak at-bat is completed — showing transition.');
      cancelAutoPlayPitch();
      activeAbEnded = true;
      if (pitchesList.length > 0) {
...
function formatStreakDifficultyMeta(ab) {
  if (!ab || ab.difficulty == null) {
    return { label: 'Standard', score: 50 };
  }
  const score = Math.max(0, Math.min(100, ab.difficulty));
  const bands = [
    [20, 'Easy'],
    [40, 'Moderate'],
    [60, 'Challenging'],
...
function updateAbStartStreakDifficultyUi(ab) {
  const textEl = document.getElementById('ab-start-streak-difficulty-text');
  const fillEl = document.getElementById('ab-start-streak-difficulty-fill');
  const { label, score } = formatStreakDifficultyMeta(ab);
  if (textEl) textEl.textContent = label;
  if (fillEl) fillEl.style.width = `${score}%`;
}

function advanceStreakToNextAtBat() {
  streakFlowEpoch++;
  cancelPendingStreakFlowTimers();
  hideStreakTransitionScreen();
  syncStreakSessionOverFromHistory();
...
async function showStreakTransitionScreen(outcomeText) {
  advanceStreakToNextAtBat();
}

function hideStreakTransitionScreen() {
  if (!streakTransitionOverlay) return;
  streakTransitionOverlay.classList.add('opacity-0', 'pointer-events-none');
  streakTransitionOverlay.classList.remove('opacity-100', 'pointer-events-auto');
  const panel = streakTransitionOverlay.querySelector('.streak-transition-panel');
  if (panel) {
...
function continueStreakFromTransition() {
  hideStreakTransitionScreen();
  activeAbEnded = false;
  currentStreakAbMeta = null;
  loadNextStreakAtBat(false);
...
async function forfeitStreakRun() {
  if (gameMode !== 'daily_streak' || isSessionOver) return;
  const username = localStorage.getItem('ump_username');
  if (username) {
    try {
      const stats = await getGlobalUserStats(username);
      stats.dnfs = (stats.dnfs || 0) + 1;
      await saveGlobalUserStats(username, stats);
      await clearActiveSession(username);
      const saved = readSavedChallengeData(username);
      if (saved?.activeChallenge?.gameMode === 'daily_streak') {
        saved.activeChallenge = null;
        saved.streakSessionUsedIds = [];
        localStorage.setItem(getChallengeStorageKey(username), JSON.stringify(saved));
      }
    } catch (e) {
      console.warn('Failed to record streak forfeit:', e);
    }
  }
  streakSessionUsedIds = new Set();
  currentStreakAbMeta = null;
...
/** @deprecated Legacy pool — streak uses STREAK_POOL_ABS via streak-rotation.js */
function extractAllAtBatsForStreak() {
  allStreakAtBatsPool = [];
  console.warn('extractAllAtBatsForStreak is deprecated; use streak pool builder.');
}

/** @deprecated Use loadNextStreakAtBat */
function loadStreakAtBat(abIdx, isResume = false) {
  activeStreakAbIndex = abIdx;
  loadNextStreakAtBat(isResume);
}
...
function parseNumericAtBatRef(value) {
...
function buildGamedayAtBatUrl(gamePk, inning, isTop, atBatNumber = null) {
  if (!gamePk || inning == null) return null;
  const half = isTop ? 'top' : 'bottom';
  const abRef = parseNumericAtBatRef(atBatNumber);
  const abSegment = abRef != null ? `/${abRef}` : '';
  return `https://www.mlb.com/gameday/${gamePk}#/game/${gamePk}/${half}/${inning}${abSegment}/`;
}

function extractGamePkFromUrl(url) {
  if (!url) return null;
  const match = String(url).match(/(?:game_id=|\/game\/)(\d+)/);
  return match ? match[1] : null;
}

function extractGamePkFromAbMeta(abMeta) {
  if (!abMeta) return null;
  if (abMeta.gamePk) return String(abMeta.gamePk);
  return extractGamePkFromUrl(abMeta.filmRoomUrl || abMeta.film_room_url)
    || extractGamePkFromUrl(abMeta.umpScorecardUrl || abMeta.ump_scorecard_url);
}

function applySummaryExternalLink(anchor, url) {
  if (!anchor) return;
  if (url) {
    anchor.href = url;
...
function findStreakPoolAb(meta) {
  if (!meta) return null;
  if (meta.id) {
    const byId = getAbById(meta.id);
    if (byId) return byId;
  }
  const pitcher = meta.pitcher;
  const batter = meta.batter;
  const inning = meta.inning ?? meta.pitches?.[0]?.inning;
  const isTop = meta.is_top ?? meta.pitches?.[0]?.is_top;
  if (!pitcher || !batter || inning == null) return null;
  return getStreakPool().find((ab) => {
    const p0 = ab.pitches?.[0];
    const abInning = ab.inning ?? p0?.inning;
    const abTop = ab.is_top ?? p0?.is_top;
    return ab.pitcher === pitcher && ab.batter === batter && abInning === inning && abTop === isTop;
  }) || null;
}

function resolveStreakAbContextForUrls() {
  const sessionIds = [...streakSessionUsedIds];
  const poolId = currentStreakAbMeta?.id || sessionIds[sessionIds.length - 1] || null;
  let poolEntry = poolId ? getAbById(poolId) : null;
  if (!poolEntry) poolEntry = findStreakPoolAb(currentStreakAbMeta);
  const session = currentStreakAbMeta || poolEntry;
  if (!session && !poolEntry) return null;

  const pitches = session?.pitches?.length ? session.pitches : poolEntry?.pitches;
  const gamePk = poolEntry?.gamePk || session?.gamePk || extractGamePkFromAbMeta(poolEntry || session);
  const inning = poolEntry?.inning ?? session?.inning ?? pitches?.[0]?.inning;
  const is_top = poolEntry?.is_top ?? session?.is_top ?? pitches?.[0]?.is_top;
  const atBatNumber = poolEntry?.atBatNumber ?? session?.atBatNumber;

  return {
    ...(poolEntry || {}),
    ...(session || {}),
    id: poolEntry?.id || session?.id,
    gamePk,
    inning,
    is_top,
    atBatNumber,
    pitches,
    umpScorecardUrl: poolEntry?.umpScorecardUrl || session?.umpScorecardUrl || session?.ump_scorecard_url,
    filmRoomUrl: poolEntry?.filmRoomUrl || session?.filmRoomUrl || session?.film_room_url,
  };
}

function getStreakSummaryExternalUrls(pitch) {
  const ab = resolveStreakAbContextForUrls();
  if (!ab) return { filmRoomUrl: null, umpScorecardUrl: null };

  const p = pitch || ab.pitches?.[ab.pitches.length - 1] || ab.pitches?.[0];
  const gamePk = extractGamePkFromAbMeta(ab);
  const inning = p?.inning ?? ab.inning ?? null;
  const isTop = p?.is_top ?? p?.isTop ?? ab.is_top ?? ab.isTop ?? true;

  const film = buildGamedayAtBatUrl(gamePk, inning, isTop, ab.atBatNumber);

  let ump = ab.umpScorecardUrl || ab.ump_scorecard_url || null;
  if (ump && isGenericExternalUrl(ump, 'ump')) ump = null;
  if (!ump && gamePk) ump = buildUmpScorecardUrl(gamePk);

  return { filmRoomUrl: film, umpScorecardUrl: ump };
}
...
async function showStreakSummaryScreen() {
  ...
  if (abStartOverlay?.classList.contains('opacity-100')) {
    setAbStartOverlayActive(false);
    abStartOverlay.classList.add('opacity-0', 'pointer-events-none');
    abStartOverlay.classList.remove('opacity-100', 'pointer-events-auto');
    const startPanel = abStartOverlay.querySelector('.ab-start-cabinet');
  ...
  const umpCorrectCount = abPitches.filter(x => x.realCorrect).length;
  const umpAccuracy = abPitches.length > 0 ? Math.round((umpCorrectCount / abPitches.length) * 100) : 100;
  ...
  if (streakSummaryUmpAccuracy) streakSummaryUmpAccuracy.textContent = `${umpAccuracy}%`;
  if (streakSummaryUmpFraction) streakSummaryUmpFraction.textContent = `${umpCorrectCount}/${abPitches.length} Correct`;
  ...
  if (username) {
    ...
    getGlobalUserStats(username).then(stats => {
      ...
      if (streakSummaryXpEarned) {
        ...
        streakSummaryXpEarned.disabled = false;
        streakSummaryXpEarned.title = 'Tap for XP breakdown';
      }
    ...
    if (streakSummaryXpEarned) {
      streakSummaryXpEarned.textContent = 'Log in for XP';
      streakSummaryXpEarned.disabled = true;
      streakSummaryXpEarned.title = '';
    }
    if (streakSummaryXpLevel) {
  ...
  const urls = getStreakSummaryExternalUrls(targetPitch);
  applySummaryExternalLink(streakSummaryFilmLink, urls.filmRoomUrl);
  applySummaryExternalLink(streakSummaryScorecardLink, urls.umpScorecardUrl);
...
async function updateAbSummaryLeaderboardSnippet(username) {
  ...
  try {
    const meta = weeklyChallengeMeta || resolveWeeklyChallengeMeta(WEEKLY_CHALLENGE_DATA, WEEKLY_CHALLENGE_META);
    const { rows } = await getLeaderboardRows('weekly', username, getActiveWeeklyLeaderboardPeriodKey());
    const me = rows.find((r) => r.isUser);
...
async function advanceNextAtBat() {
  dismissAbSummaryOverlay();
  cancelAutoPlayPitch();
  setCameraAngle('umpire');
  ...
  if (gameMode === 'weekly_challenge' || gameMode === 'mlb_game') {
    if (gameMode === 'weekly_challenge') {
      finalizeWeeklyAbPlaylistEntry(activeWeeklyAbIndex);
      const target = getWeeklyChallengeTargetAtBats();
      const completedAbs = weeklyPlaylistABs.filter((ab) => ab.completed).length;
      if (completedAbs >= target || activeWeeklyAbIndex >= weeklyPlaylistABs.length - 1) {
        void finishWeeklyChallenge();
        return;
      }
    }
...
function saveChallengeSessionToLocal() {
  ...
  const meta = weeklyChallengeMeta || resolveWeeklyChallengeMeta(WEEKLY_CHALLENGE_DATA, WEEKLY_CHALLENGE_META);
  const weeklyComplete = isWeeklyChallengeRunComplete(sanitizedPlaylist);
  const data = {
    savedAt: Date.now(),
    completedABsCount,
    weeklyPlaylistABs: sanitizedPlaylist,
    weeklyChallengeWeekId: meta.challengeWeekId,
    weeklyTargetAtBats: getWeeklyChallengeTargetAtBats(),
    weeklyRunComplete: weeklyComplete,
    weeklyRunCompletedAt: weeklyComplete ? Date.now() : null,
    weeklyBundleRevision: getStoredWeeklyRevisionKey(meta.challengeWeekId, activeWeeklyBundleId),
  ...
  if (shouldSnapshotChallenge && (hasLivePitchState || !onMenuScreen)) {
    data.activeChallenge = {
      ...
      streakSessionUsedIds: [...streakSessionUsedIds],
      streakRunSeedValue,
...
function isWeeklyChallengeRunComplete(playlist = weeklyPlaylistABs) {
  ...
  const target = getWeeklyChallengeTargetAtBats();
  let completed = 0;
  for (const ab of playlist) {
    if (ab.completed) {
      completed += 1;
      continue;
    }
    const pitches = getAbPitches(ab);
    if (isWeeklyAbFullyComplete(ab, pitches)) completed += 1;
  }
  return completed >= target;
}

function getActiveWeeklyLeaderboardPeriodKey() {
  const meta = weeklyChallengeMeta || resolveWeeklyChallengeMeta(WEEKLY_CHALLENGE_DATA, WEEKLY_CHALLENGE_META);
  return getWeeklyLeaderboardPeriodKey(meta.challengeWeekId, meta.resetDate);
}

function finalizeWeeklyAbPlaylistEntry(abIndex) {
  const ab = weeklyPlaylistABs[abIndex];
  if (!ab) return;
  if (abIndex === activeWeeklyAbIndex) {
    syncWeeklyPlaylistFromActiveAtBat();
  }
  const pitches = getAbPitches(ab);
  const called = pitches.filter((p) => p.userCall !== undefined && !p.isSwingPlay);
  ab.userCorrectCount = called.filter((p) => p.userCorrect).length;
  ab.userTotalCount = called.length;
  if (isWeeklyAbFullyComplete(ab, pitches)) {
    ab.completed = true;
  }
}

async function finishWeeklyChallenge() {
  ...
  for (let i = 0; i < weeklyPlaylistABs.length; i++) {
    finalizeWeeklyAbPlaylistEntry(i);
  }
  ...
  if (username) {
    try {
      ...
      console.warn('Failed to clear weekly challenge session after completion:', e);
    }
  }
...
function weeklyPitchProgressKey(pitch) {
  if (!pitch) return '';
  if (pitch.id != null) return `id:${pitch.id}`;
  return `ab:${pitch.at_bat_number ?? ''}:${pitch.pitch_number ?? ''}:${pitch.batter ?? ''}:${pitch.pitcher ?? ''}`;
}

function weeklyAbProgressKey(ab) {
  const p0 = getAbPitches(ab)[0];
  if (p0) return weeklyPitchProgressKey(p0);
  return `gi:${ab?.gameIndex ?? ''}:${ab?.batter ?? ''}:${ab?.pitcher ?? ''}`;
}

function applyWeeklyPitchCallFields(target, source) {
  if (!target || !source || source.userCall === undefined) return;
  target.userCall = source.userCall;
  target.absCall = source.absCall;
  target.realCall = source.realCall;
  target.userCorrect = source.userCorrect;
  target.realCorrect = source.realCorrect;
  target.isSwingPlay = !!source.isSwingPlay;
  if (source.swingOutcome != null) target.swingOutcome = source.swingOutcome;
  if (source.swingHitType != null) target.swingHitType = source.swingHitType;
}

function weeklyPlaylistHasProgress(playlist) {
  if (!playlist?.length) return false;
  return playlist.some(
    (ab) =>
      ab.completed ||
      getAbPitches(ab).some((p) => p.userCall !== undefined)
  );
}
...
function mergeWeeklyAbEntry(freshAb, savedAb) {
  const freshPitches = (freshAb.pitches || []).map((p) => ({ ...p }));
  if (!savedAb) {
    return {
      ...freshAb,
      pitches: freshPitches,
      completed: !!freshAb.completed,
      userCorrectCount: freshAb.userCorrectCount || 0,
      userTotalCount: freshAb.userTotalCount || 0,
    };
  }

  const savedPitches = getAbPitches(savedAb);
  const savedPitchMap = new Map();
  savedPitches.forEach((p) => savedPitchMap.set(weeklyPitchProgressKey(p), p));

  const mergedPitches = freshPitches.map((fp) => {
    const fromSaved = savedPitchMap.get(weeklyPitchProgressKey(fp));
    if (!fromSaved) return fp;
    const merged = { ...fp };
    applyWeeklyPitchCallFields(merged, fromSaved);
    return merged;
  });
  ...
  return {
    ...
    gameTitle: freshAb.gameTitle || savedAb.gameTitle,
    filmRoomUrl: freshAb.filmRoomUrl || savedAb.filmRoomUrl,
    umpScorecardUrl: freshAb.umpScorecardUrl || savedAb.umpScorecardUrl,
    batter: freshAb.batter || savedAb.batter,
    pitcher: freshAb.pitcher || savedAb.pitcher,
  };
}

/** Merge saved user calls / AB completion onto the current week's fresh playlist (by pitch id + index). */
function mergeWeeklyPlaylistProgress(freshPlaylist, savedPlaylist) {
  if (!freshPlaylist?.length) return [];
  if (!savedPlaylist?.length) {
    return freshPlaylist.map((ab) => ({
      ...ab,
      pitches: (ab.pitches || []).map((p) => ({ ...p })),
      completed: !!ab.completed,
      userCorrectCount: ab.userCorrectCount || 0,
      userTotalCount: ab.userTotalCount || 0,
    }));
  }

  const savedByAbKey = new Map();
  savedPlaylist.forEach((ab) => savedByAbKey.set(weeklyAbProgressKey(ab), ab));

  return freshPlaylist.map((freshAb) => {
    const savedAb = savedByAbKey.get(weeklyAbProgressKey(freshAb));
    return mergeWeeklyAbEntry(freshAb, savedAb);
  });
}
...
function checkWeeklyAbIsFinished(ab, pitches = getAbPitches(ab)) {
  ...
  for (const pitch of pitches) {
    if (pitch.userCall === undefined) return false;
    if (pitch.isSwingPlay) {
      if (pitch.swingOutcome === 'WHIFF') strikes++;
      else if (pitch.swingOutcome === 'FOUL' && strikes < 2) strikes++;
      else if (pitch.swingOutcome === 'HIT' || pitch.swingOutcome === 'OUT') return true;
    } else if (pitch.userCall === 'S' || pitch.userCall === 'TIMEOUT') {
      strikes++;
    } else if (pitch.userCall === 'B') {
      balls++;
    }
...
function syncWeeklyPlaylistFromActiveAtBat() {
  if (gameMode !== 'weekly_challenge' && gameMode !== 'mlb_game') return;
  const ab = weeklyPlaylistABs[activeWeeklyAbIndex];
  if (!ab) return;
  const pitches = getAbPitches(ab);
  pitches.forEach((pitch, i) => {
    const live = pitchesList[i];
    if (live?.userCall !== undefined) {
      applyWeeklyPitchCallFields(pitch, live);
    }
  });
  pitchHistory.forEach((item, idx) => {
    const pitch = pitches[idx];
    if (!pitch) return;
    const src = item.pitchData || item;
    if (src.userCall !== undefined) {
      applyWeeklyPitchCallFields(pitch, src);
    }
  });
}

function pickNewerChallengeProgress(cloud, local) {
  if (!cloud?.weeklyPlaylistABs?.length) return local?.weeklyPlaylistABs?.length ? local : null;
  if (!local?.weeklyPlaylistABs?.length) return cloud;
  const cloudTs = Number(cloud.savedAt) || 0;
  const localTs = Number(local.savedAt) || 0;
  return cloudTs >= localTs ? cloud : local;
}

function challengeProgressMatchesWeek(progress, weekId) {
  if (!progress?.weeklyPlaylistABs?.length) return false;
  if (!progress.weeklyChallengeWeekId) return true;
  return progress.weeklyChallengeWeekId === weekId;
}

function getCurrentWeeklyBundleRevisionKey() {
  const meta = weeklyChallengeMeta || resolveWeeklyChallengeMeta(WEEKLY_CHALLENGE_DATA, WEEKLY_CHALLENGE_META);
  return getStoredWeeklyRevisionKey(meta.challengeWeekId, activeWeeklyBundleId);
}

function challengeProgressMatchesBundle(progress) {
  if (!progress?.weeklyPlaylistABs?.length) return false;
  const expected = getCurrentWeeklyBundleRevisionKey();
  if (!progress.weeklyBundleRevision) {
    const storedRevision = readStoredWeeklyRevision();
    return !storedRevision || storedRevision === expected;
  }
  return progress.weeklyBundleRevision === expected;
}
...
async function refreshWeeklyChallengeBundleIfNeeded() {
  ...
  const username = localStorage.getItem('ump_username');
  const revisionKey = getCurrentWeeklyBundleRevisionKey();
  const storedRevision = readStoredWeeklyRevision();
  if (storedRevision && storedRevision !== revisionKey) {
    console.log(
      'Weekly challenge playlist revision changed — resetting progress.',
      storedRevision,
      '->',
      revisionKey
    );
...
async function getSavedWeeklyPlaylistForMerge(username) {
  const meta = weeklyChallengeMeta || resolveWeeklyChallengeMeta(WEEKLY_CHALLENGE_DATA, WEEKLY_CHALLENGE_META);
  const weekId = meta.challengeWeekId;
  const localCp = challengeProgressMatchesWeek(readSavedChallengeData(username), weekId)
    ? readSavedChallengeData(username)
    : null;
  let cloudCp = null;
  if (username) {
    try {
  ...
  if (!bundle?.weeklyPlaylistABs?.length) return [];
  if (!challengeProgressMatchesBundle(bundle)) return [];
  return bundle.weeklyPlaylistABs;
}

async function syncWeeklyPlaylistForMenuDisplay() {
  const fresh = extractAtBatsFromWeeklyData();
  const username = localStorage.getItem('ump_username');
  const saved = await getSavedWeeklyPlaylistForMerge(username);
  weeklyPlaylistABs = mergeWeeklyPlaylistProgress(fresh, saved);
}

function dismissAbSummaryOverlay() {
  if (!abSummaryOverlay) return;
  abSummaryOverlay.classList.add('opacity-0', 'pointer-events-none');
  abSummaryOverlay.classList.remove('opacity-100', 'pointer-events-auto');
  const summaryPanel = abSummaryOverlay.querySelector('.ab-summary-panel');
...
/**
 * Restore weekly playlist + AB index from saved progress (playlist pitches are source of truth).
 */
async function resolveWeeklyChallengeResume(rawABs, username) {
  const freshPlaylist = rawABs?.length ? rawABs : [];
  const currentWeekId = (weeklyChallengeMeta || resolveWeeklyChallengeMeta(WEEKLY_CHALLENGE_DATA, WEEKLY_CHALLENGE_META))
    .challengeWeekId;
  ...
  const localCp =
    challengeProgressMatchesWeek(localData, currentWeekId) && challengeProgressMatchesBundle(localData)
      ? localData
      : null;
  const cloudCp =
    challengeProgressMatchesWeek(cloudData, currentWeekId) && challengeProgressMatchesBundle(cloudData)
      ? cloudData
      : null;
  const savedBundle = pickNewerChallengeProgress(cloudCp, localCp);
...
function hasWeeklyChallengeResumeProgress() {
  if (isWeeklyChallengeRunComplete(weeklyPlaylistABs)) return false;
  if (weeklyPlaylistHasProgress(weeklyPlaylistABs)) return true;
  const saved = readSavedChallengeData();
  if (weeklyPlaylistHasProgress(saved?.weeklyPlaylistABs)) return true;
...
async function loadSavedSessionFromLocal() {
  ...
  const storedWeekId = localStorage.getItem('ump_weekly_challenge_week') || '';
  ...
  if (storedWeekId && storedWeekId !== currentWeekId) {
    console.log(
      'Weekly challenge week changed — resetting progress.',
      'was:',
      storedWeekId,
      'now:',
      currentWeekId
  ...
  } else if (storedRevision && storedRevision !== revisionKey) {
    console.log(
      'Weekly challenge playlist revision changed — resetting progress.',
      storedRevision,
      '->',
    ...
    if (session && !weeklyWeekJustReset) {
      if (session.weeklyPlaylistABs?.length) {
        ...
        if (session.gameMode === 'weekly_challenge') {
          activeWeeklyAbIndex = findWeeklyResumeAbIndex(
            weeklyPlaylistABs,
            session.activeWeeklyAbIndex
          );
        }
      }
  ...
  if (rawWeekly) {
    try {
      ...
      if (data.weeklyPlaylistABs && data.weeklyPlaylistABs.length > 0) {
        weeklyPlaylistABs = mergeWeeklyPlaylistProgress(freshWeekly, data.weeklyPlaylistABs);
      } else if (!weeklyPlaylistABs.length) {
        weeklyPlaylistABs = freshWeekly;
      }
  ...
  if (username && !weeklyWeekJustReset) {
    try {
      const stats = await getGlobalUserStats(username);
      const cloudCp = stats?.challengeProgress;
      const localCp = readSavedChallengeData(username);
      const bundle = pickNewerChallengeProgress(
        challengeProgressMatchesWeek(cloudCp, currentWeekId) ? cloudCp : null,
        challengeProgressMatchesWeek(localCp, currentWeekId) ? localCp : null
      );
      if (
        bundle?.weeklyPlaylistABs?.length &&
        challengeProgressMatchesBundle(bundle)
      ) {
        const freshWeekly = extractAtBatsFromWeeklyData();
        weeklyPlaylistABs = mergeWeeklyPlaylistProgress(freshWeekly, bundle.weeklyPlaylistABs);
        activeWeeklyAbIndex = findWeeklyResumeAbIndex(
          weeklyPlaylistABs,
          bundle.activeWeeklyAbIndex
        );
        if (bundle.completedABsCount) completedABsCount = bundle.completedABsCount;
        saveChallengeSessionToLocal();
      }
    } catch (e) {
      console.warn('Failed to apply cloud weekly challenge progress:', e);
    }
  }
  ...
  try {
    if (mode === 'weekly_challenge') {
      ...
      if (hasOfficialWeeklyLeaderboardStanding()) {
        ...
        const me = rows.find((r) => r.isUser);
        pauseChallengeRank.textContent = me ? `#${me.rank} · ${me.accuracy}` : 'Complete — syncing rank';
      } else {
        const snapshot = getWeeklyChallengeProgressSnapshot();
        const scoreRaw = computeWeeklyLeaderboardScoreRaw();
        if (scoreRaw > 0) {
          const { rank } = await estimateWeeklyLeaderboardPlacement(username, scoreRaw, periodKey);
          pauseChallengeRank.textContent = `~#${rank} projected · ${snapshot.sessionAccuracy ?? '--'}%`;
        } else {
          pauseChallengeRank.textContent = `Finish ${snapshot.totalCount} ABs to rank`;
        }
      }
...
function pauseGameOnFocusLoss() {
  ...
  if (streakTransitionOverlay && streakTransitionOverlay.classList.contains('opacity-100')) return;
  if (streakSummaryOverlay && streakSummaryOverlay.classList.contains('opacity-100')) return;
  ...
  if (isTransitioningToSummary && activeAbEnded) {
    if (isStreakBetweenAbMenuVisible()) return;
    if (summaryTimeout) {
      clearTimeout(summaryTimeout);
      summaryTimeout = null;
    }
...
async function updateChallengeProgressUI() {
  ...
  if (btnStartWeeklyChallenge && !btnStartWeeklyChallenge.classList.contains('ump-btn--busy')) {
    const runComplete = isWeeklyChallengeRunComplete(weeklyPlaylistABs);
    const hasProgress = hasWeeklyChallengeResumeProgress();
    const label = runComplete
      ? 'View Results'
      : hasProgress && completedCount < total
        ? 'Resume Challenge'
        : 'Start Challenge';
...
function updateSummaryTimerUI() {
  ...
  const isEnd = (gameMode === 'weekly_challenge')
    ...
    : isSessionOver;
  if (isEnd && gameMode === 'weekly_challenge') {
    btnAbSummaryAdvance.textContent = 'FINISH CHALLENGE';
  } else {
...
async function startWeeklyChallenge() {
  ...
  if (
    activeWeeklyAbIndex >= weeklyPlaylistABs.length ||
    isWeeklyChallengeRunComplete(weeklyPlaylistABs)
  ) {
    await finishWeeklyChallenge();
    return;
  }

  const resumeAb = weeklyPlaylistABs[activeWeeklyAbIndex];
  if (!getAbPitches(resumeAb).length) {
  ...
  loadWeeklyAtBat(resumeAbIdx, {
    ...
    if (toastMessage) {
      toastMessage.innerHTML =
        '<span class="text-amber-300 font-bold font-mono-tech">Could not resume weekly challenge — try again</span>';
      toastMessage.classList.remove('opacity-0', 'scale-95', '-translate-y-4');
      toastMessage.classList.add('opacity-100', 'scale-100', 'translate-y-0');
      setTimeout(() => {
        toastMessage.classList.add('opacity-0', 'scale-95', '-translate-y-4');
...
function determineAbOutcomeFromHistory() {
  ...
  if (gameMode === 'weekly_challenge') {
    const lastPitch = pitchesList[pitchesList.length - 1];
    return formatWeeklyAbOutcomeText(lastPitch, { balls: abBalls, strikes: abStrikes });
  }
...
function getCurrentChallengeWeekId() {
  const meta = weeklyChallengeMeta || resolveWeeklyChallengeMeta(WEEKLY_CHALLENGE_DATA, WEEKLY_CHALLENGE_META);
  return getActiveWeeklyLeaderboardPeriodKey();
}
...
function parseLeaderboardScoreRaw(row) {
  if (!row) return 0;
  const fromText = String(row.score || '').match(/(\d+)/);
  if (fromText) return Number(fromText[1]) || 0;
  const direct = Number(row.scoreRaw);
  return Number.isFinite(direct) ? direct : 0;
}

function computeWeeklyLeaderboardScoreRaw() {
  const { completedCount } = getWeeklyChallengeCompletedStats();
  if (!completedCount && gameMode !== 'weekly_challenge') return 0;

  let correct = 0;
  weeklyPlaylistABs?.forEach((ab) => {
    if (ab.completed) correct += ab.userCorrectCount || 0;
  });
  if (gameMode === 'weekly_challenge') {
    correct += pitchHistory.filter((x) => !x.isSwingPlay && x.userCorrect).length;
  }
  return correct * 10;
}

function hasOfficialWeeklyLeaderboardStanding() {
  if (isWeeklyChallengeRunComplete(weeklyPlaylistABs)) return true;
  const username = localStorage.getItem('ump_username');
  if (!username) return false;
  const saved = readSavedChallengeData(username);
  const meta = weeklyChallengeMeta || resolveWeeklyChallengeMeta(WEEKLY_CHALLENGE_DATA, WEEKLY_CHALLENGE_META);
  if (!challengeProgressMatchesWeek(saved, meta.challengeWeekId)) return false;
  return isWeeklyProgressCompleteForLeaderboard(saved);
}

async function estimateWeeklyLeaderboardPlacement(username, scoreRaw, periodKey) {
  const { rows } = await getLeaderboardRows('weekly', username, periodKey);
  let rank = 1;
  for (const row of rows) {
    if (!row.isUser && parseLeaderboardScoreRaw(row) > scoreRaw) rank += 1;
  }
  return { rank, rowCount: rows.length };
}

function formatWeeklyInProgressStandingsLine(snapshot, projectedRank = null) {
  const { sessionAccuracy, completedCount, totalCount } = snapshot;
  const accPart =
    sessionAccuracy != null ? `${sessionAccuracy}% session accuracy` : 'No calls yet';
  const progressPart = `${completedCount} / ${totalCount} at-bats done`;
  const rankPart = projectedRank
    ? ` · ~#${projectedRank} if you finish now (not ranked until complete)`
    : ' · Finish all at-bats to appear on the leaderboard';
  return `${accPart} · ${progressPart}${rankPart}`;
}
...
async function submitGlobalScore(type, name, team, accuracy, scoreValue, rawScore) {
  ...
  if (board === 'weekly' && !isWeeklyChallengeRunComplete(weeklyPlaylistABs)) {
    return;
  }
  try {
    await apiSubmitLeaderboard({
      board,
      periodKey: board === 'weekly' ? getActiveWeeklyLeaderboardPeriodKey() : undefined,
      team: team || 'None',
      accuracy,
...
async function populateWeeklyChallengeDetailModal() {
  ...
  const explained = explainWeeklyPlaylist(WEEKLY_CHALLENGE_DATA, meta);
  const playlistWrap = document.getElementById('challenge-detail-playlist-wrap');
  const playlistList = document.getElementById('challenge-detail-playlist-list');
  const playlistMeta = document.getElementById('challenge-detail-playlist-meta');
  ...
  if (challengeDetailDesc) {
    challengeDetailDesc.textContent =
      'Full MLB plate appearances from this week\'s curated games. Call every pitch — each at-bat ends with the real result (walk, strikeout, or ball in play). Earn 10 points per correct call.';
  }
  ...
  if (challengeDetailGamesList) {
    challengeDetailGamesList.innerHTML = WEEKLY_CHALLENGE_DATA.slice(0, meta.gameCount)
      .map((game, idx) => {
  ...
  if (btnChallengeDetailPlay) {
    btnChallengeDetailPlay.textContent = isWeeklyChallengeRunComplete(weeklyPlaylistABs)
      ? 'View Results'
      : hasWeeklyChallengeResumeProgress()
        ? 'Resume Challenge'
        : 'Start Challenge';
  ...
  if (btnChallengeDetailStandings) {
    ...
    btnChallengeDetailStandings.onclick = (e) => {
      e.stopPropagation();
      hideChallengeDetailModal();
      openStandingsTab({ board: 'weekly', view: 'ranks', periodKey: getActiveWeeklyLeaderboardPeriodKey() });
    };
  }
...
async function populateStreakChallengeDetailModal() {
  const username = localStorage.getItem('ump_username') || 'GUEST_UMPIRE';
  ...
  if (challengeDetailCompleted) {
    challengeDetailCompleted.textContent =
      document.getElementById('daily-streak-outcome')?.textContent?.replace(/\s*PITCHES/i, '') || '0';
  }
...
async function openChallengeDetailModal(type) {
  ...
  setChallengeDetailModalLoading(true);
  if (challengeDetailModalOverlay) {
  ...
  try {
    if (type === 'weekly') {
      await populateWeeklyChallengeDetailModal();
    } else {
      await populateStreakChallengeDetailModal();
    }
  } catch (err) {
    console.error('Challenge detail modal failed:', err);
    if (challengeDetailDesc) {
      challengeDetailDesc.textContent = 'Could not load challenge details. Try again in a moment.';
    }
  } finally {
    setChallengeDetailModalLoading(false);
  }
}
...
let activePlayerPopoutDismiss = null;

function closeAllPlayerStatsPopouts() {
  document.querySelectorAll('.player-card-popout').forEach((p) => p.remove());
  if (activePlayerPopoutDismiss) {
    document.removeEventListener('pointerdown', activePlayerPopoutDismiss, true);
    activePlayerPopoutDismiss = null;
  }
}
...
function bindPlayerPopoutClose(popout, element) {
  popout.addEventListener('click', (e) => {
    e.stopPropagation();
    if (e.target.closest('.btn-close-popout')) {
      closeAllPlayerStatsPopouts();
    }
  });
  ...
  activePlayerPopoutDismiss = (e) => {
    if (!document.body.contains(popout)) {
      document.removeEventListener('pointerdown', activePlayerPopoutDismiss, true);
      activePlayerPopoutDismiss = null;
      return;
    }
    if (popout.contains(e.target) || element.contains(e.target)) return;
    closeAllPlayerStatsPopouts();
  };

  requestAnimationFrame(() => {
    document.addEventListener('pointerdown', activePlayerPopoutDismiss, true);
  });
}
