 ...
 * data file. Leaderboard rollover is handled by Supabase period keys (ISO week).
 *
...
import { getIsoWeekKey, getMondayDateString, isoWeekToShuffleSeed } from '../src/js/challenge-utils.js';
...
function writeDataFile(games) {
  ...
  const challengeWeekId = getIsoWeekKey(today);
  const gamePks = games.map((g) => g.gamePk).filter(Boolean);

  const meta = {
    challengeWeekId,
    resetDate: getMondayDateString(today),
    gameCount: games.length,
    targetAtBats: 200,
    gamePks,
    shuffleSeed: isoWeekToShuffleSeed(challengeWeekId),
  };
